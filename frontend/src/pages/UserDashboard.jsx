...
export default function UserDashboard() {
  const [selection, setSelection] = useState({ gouvernoratId: null, delegationId: null, secteurId: null })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 1280 }}>
      <div>
        ...
        <p style={{ color: 'var(--text-muted)', margin: 0 }}>
          Selectionnez un gouvernorat/delegation pour une note globale, ou un secteur pour le detail.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'stretch' }}>
        ...
        <div style={{ flex: 1 }}>
          <MapView
            secteurId={secteurId} delegationId={delegationId} gouvernoratId={gouvernoratId}
            operator={operatorFilter} height={320}
            layerKey={layerKey} showBadPoints={showBadPoints} onBadPointsChange={setBadPointsCount}
          />
        </div>
      </div>
      ...
      <div>
        <div className="eyebrow" style={{ marginBottom: 10 }}>
          Comparaison par operateur {scope === 'delegation' ? '(moyenne de la delegation)' : ''}
        </div>
